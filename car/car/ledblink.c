...
osThreadId_t blink_task_ID; // 任务1

#define DELAY_TIME_MS 50 // 延时时间，单位为毫秒
int blink_task(const char *arg)
{
    unused(arg);
    // 配置引脚为普通GPIO模式
 ...
 void blink_entry(void)
{
    ...
    blink_task_ID = osThreadNew((osThreadFunc_t)blink_task, NULL, &attr);
    if (blink_task_ID != NULL) {
        printf("ID = %d, Create blink_task_ID is OK!\r\n", blink_task_ID);
    }
}
