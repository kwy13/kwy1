...
osThreadId_t ssd1306_task_ID; // 任务1
...
void ssd1306_task(void)
{
 ...
 void base_ssd1306_demo(void)
{
    printf("Enter base_sdd1306_demo()!\r\n");

    osThreadAttr_t attr;
    attr.name = "ssd1306_task";
    attr.attr_bits = 0U;
    attr.cb_mem = NULL;
    attr.cb_size = 0U;
    attr.stack_mem = NULL;
    ...
    ssd1306_task_ID = osThreadNew((osThreadFunc_t)ssd1306_task, NULL, &attr);
    if (ssd1306_task_ID != NULL) {
        printf("ID = %d, Create ssd1306_task_ID is OK!\r\n", ssd1306_task_ID);
    }
}
